...
jasmine.executeSpecsInFolder = function(folder,
  ...
  if(jsCoverageConfig.report){
    jsCoverageConfig.output = junitreport.savePath;
    var JsCoverage = require('./jscoverage').JsCoverage,
        jsCoverage = new JsCoverage(jsCoverageConfig),
        onComplete = done,
        that = this;
        
    done = function(runner, log){
        jsCoverage.coverageReport(function(){
            onComplete.call(that, runner, log);
        });        
    };
  }
  ...
  if(jsCoverageConfig.report){
      var that = this;
      jsCoverage.prepare(function(){
        execute.call(that);
      });
  } else if (useRequireJs) {
